...
WITH SeqNumTbl(SeqNum) AS 
  (SELECT --(((CURRENT DATE) - DAY(CURRENT DATE - 1 DAYS) DAYS) - MONTH(CURRENT DATE - 1 MONTHS) MONTHS) AS SeqNum
          (((DATE('2017-01-01')) - DAY(DATE('2017-01-01') - 1 DAYS) DAYS) - MONTH(DATE('2017-01-01') - 1 MONTHS) MONTHS) AS SeqNum
   ...
   WHERE SeqNum < (((DATE('2017-01-01')) - DAY(DATE('2017-01-01') - 1 DAYS) DAYS) - MONTH(DATE('2017-01-01') - 1 MONTHS) MONTHS) + 2 YEARS)   
