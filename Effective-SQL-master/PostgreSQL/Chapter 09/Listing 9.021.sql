...
WITH RECURSIVE SeqNumTbl(SeqKey, SeqNum) AS 
  (SELECT 1 AS SeqKey,
          --DATE_TRUNC('YEAR', CURRENT_DATE) AS SeqNum
		  DATE_TRUNC('YEAR', TO_DATE('2017-01-01','YYYY-MM-DD')) AS SeqNum		  
   UNION ALL
   SELECT SeqKey + 1, SeqNum + INTERVAL '1 DAYS' 
   FROM SeqNumTbl
--   WHERE SeqNum < (DATE_TRUNC('YEAR', CURRENT_DATE + INTERVAL '2 YEARS')))
   WHERE SeqNum < (DATE_TRUNC('YEAR', TO_DATE('2017-01-01','YYYY-MM-DD') + INTERVAL '2 YEARS')))   
